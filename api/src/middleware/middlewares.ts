...
export async function getEmployeeByEmailAndPassword(req: Request, res: any, next: NextFunction) {
    let employee
    try {
        employee = await Employees.findOne({email: req.body.email, password: req.body.password})
        if (employee === null) return res.status(404).json({ message: MESSAGE_CANNOT_FIND_EMPLOYEE })
    } catch (e: any) {
        return res.status(500).json({ message: e.message })
    }
